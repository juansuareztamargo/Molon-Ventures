...
export class GradientTrajectoryTest {
  ...
  private testWidthCalculations(): void {
    try {
      ...
      testCases.forEach(
        ({ powerRatio, expectedBase, expectedStart, expectedEnd }) => {
          ...
          if (
            Math.abs(baseLineWidth - expectedBase) > tolerance ||
            Math.abs(startWidth - expectedStart) > tolerance ||
            Math.abs(endWidth - expectedEnd) > tolerance
          ) {
            console.log(
              `❌ Width calculation failed for powerRatio ${powerRatio}:`,
              {
                baseLineWidth,
                expectedBase,
                startWidth,
    ...
    try {
      ...
      testCases.forEach(({ progress, expectedWidth }) => {
        ...
        if (Math.abs(segmentWidth - expectedWidth) > tolerance) {
          console.log(
            `❌ Gradient progression failed for progress ${progress}:`,
            {
              segmentWidth,
              expectedWidth,
            }
  ...
  private testSnappedWidthAdjustment(): void {
    try {
      ...
      const notSnappedCorrect =
        Math.abs(finalEndWidthNotSnapped - 4.6) <= tolerance;
      const snappedCorrect = Math.abs(finalEndWidthSnapped - 5.6) <= tolerance;

      this.testResults['snapped_width_adjustment'] =
        notSnappedCorrect && snappedCorrect;
